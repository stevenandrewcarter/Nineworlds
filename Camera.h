...
public:
    ...
    void RotateView(float angle, float X, float Y,
                    float Z);    // This rotates the camera's view around the position depending on the values passed in.
    ...
    void
    Update(HWND hWnd);                            // This updates the camera's view and other data (Should be called each frame)

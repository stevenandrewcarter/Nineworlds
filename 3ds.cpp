    ...
    while (pPreviousChunk->bytesRead < pPreviousChunk->length) {
        ...
        switch (currentChunk.ID) {
            case VERSION:                            // This holds the version of the file
                ...
                currentChunk.bytesRead += fread(gBuffer, 1, currentChunk.length - currentChunk.bytesRead,
                                                m_FilePointer);
    ...
    while (pPreviousChunk->bytesRead < pPreviousChunk->length) {
        ...
        switch (currentChunk.ID) {
            case MATNAME:                            // This chunk holds the name of the material
                ...
                currentChunk.bytesRead += fread(pModel->pMaterials[pModel->numOfMaterials - 1].strName, 1,
            ...
            case MATMAPFILE:                        // This stores the file name of the material
                ...
                currentChunk.bytesRead += fread(pModel->pMaterials[pModel->numOfMaterials - 1].strFile, 1,
                                                currentChunk.length - currentChunk.bytesRead, m_FilePointer);
                break;
...
void CLoad3DS::ReadUVCoordinates(t3DObject *pObject, tChunk *pPreviousChunk) {
    ...
    pPreviousChunk->bytesRead += fread(pObject->pTexVerts, 1, pPreviousChunk->length - pPreviousChunk->bytesRead,
                                       m_FilePointer);
}
...
void CLoad3DS::ReadVertices(t3DObject *pObject, tChunk *pPreviousChunk) {
    ...
    pPreviousChunk->bytesRead += fread(pObject->pVerts, 1, pPreviousChunk->length - pPreviousChunk->bytesRead,
                                       m_FilePointer);
    ...
    for (int index = 0; index < pModel->numOfObjects; index++) {
        ...
        for (int i = 0; i < pObject->numOfFaces; i++) {
            ...
            vVector1 = Vector(vPoly[0],
                              vPoly[2]);        // Get the vector of the polygon (we just need 2 sides for the normal)
            vVector2 = Vector(vPoly[2], vPoly[1]);        // Get a second vector of the polygon

            vNormal = Cross(vVector1,
                            vVector2);        // Return the cross product of the 2 vectors (normalize vector, but not a unit vector)

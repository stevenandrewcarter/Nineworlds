...
void SurfaceCreator::BlendSurfaceTexture() {
    ...
    for (int y = 0; y < Surface.getheight(); y++)                            //Go through all pixels
    {
        for (int x = 0; x < Surface.getwidth(); x++) {
            ...
            new_r = (unsigned) Limit255((r[0] * f0) + (r[1] * f1) + (r[2] * f2) +
                                        (r[3] * f3));    //Calculate new r value for the pixel to be written
            new_g = (unsigned) Limit255((g[0] * f0) + (g[1] * f1) + (g[2] * f2) +
                                        (g[3] * f3));    //Calculate new g value for the pixel to be written
            new_b = (unsigned) Limit255((b[0] * f0) + (b[1] * f1) + (b[2] * f2) +
                                        (b[3] * f3));    //Calculate new b value for the pixel to be written
...
void SurfaceCreator::SmoothShadowMap() {
    ...
    for (int x = 1; x < T_Size - 1; x++) {
        for (int y = 1; y < T_Size - 1; y++) {
            ...
            Shadows.getcolor(x + 1, y + 1, r[8], g[8],
                             b[8]);                    //Get color of pixel x/z from bitmap2
